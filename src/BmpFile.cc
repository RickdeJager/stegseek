...
void BmpFile::readheaders() {
    try {
        ...
        default: {
            if (getBinIO()->is_std()) {
                throw NotImplementedError(_("the bmp data from standard input has a format that is "
                                            "not supported (biSize: %lu)."),
                                          tmpSize);
            } else {
                throw NotImplementedError(
                    _("the bmp file \"%s\" has a format that is not supported (biSize: %lu)."),
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while reading the bmp headers from the file \"%s\"."),
        ...
        case BinaryInputError::STDIN_ERR: {
            throw SteghideError(
                _("an error occurred while reading the bmp headers from standard input."));
            break;
        }

        case BinaryInputError::STDIN_EOF: {
            throw SteghideError(
                _("premature end of data from standard input while reading bmp headers."));
            break;
        }
        }
    }
...
void BmpFile::writeheaders() {
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while writing the bmp headers to the file \"%s\"."),
                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryOutputError::STDOUT_ERR: {
            throw SteghideError(
                _("an error occurred while writing the bmp headers to standard output."));
            break;
        }
        }
    }
    ...
    try {
        ...
        for (unsigned long line = 0; line < height; line++) {
            ...
            for (int i = 0; i < paddinglength; i++) {
                if (getBinIO()->read8() != 0) {
                    Warning w(
                        _("maybe corrupted bmp data (padding byte at 0x%lx set to non-zero)."),
                        getBinIO()->getPos() - 1);
                    w.printMessage();
                }
            }
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while reading the bmp data from the file \"%s\"."),
                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryInputError::STDIN_ERR: {
            throw SteghideError(
                _("an error occurred while reading the bmp data from standard input."));
            break;
        }
...
void BmpFile::writedata() {
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while writing the bitmap data to the file \"%s\"."),
                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryOutputError::STDOUT_ERR: {
            throw SteghideError(
                _("an error occurred while writing the bitmap data to standard output."));
            break;
        }
        }
    }
