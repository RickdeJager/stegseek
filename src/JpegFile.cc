...
void JpegFile::read(BinaryIO *io) {
    ...
    // TODO - use BinaryIO (or similar class) as input/output module for libjpeg
    // (to avoid the NotImplementedError and using FILE* here)
    FILE *infile = NULL;
    if (io->is_std()) {
        throw NotImplementedError(
            _("can not use standard input as source for jpeg files with libjpeg."));
    } else {
        infile = io->getStream();
        rewind(infile);
    }
    ...
    for (unsigned short icomp = 0; icomp < DeCInfo.num_components; icomp++) {
        max_v_samp_factor =
            AUtils::max<unsigned short>(max_v_samp_factor, DeCInfo.comp_info[icomp].v_samp_factor);
        max_h_samp_factor =
            AUtils::max<unsigned short>(max_h_samp_factor, DeCInfo.comp_info[icomp].h_samp_factor);
    }
    HeightInBlocks = new unsigned int[DeCInfo.num_components];
    WidthInBlocks = new unsigned int[DeCInfo.num_components];
    for (unsigned short icomp = 0; icomp < DeCInfo.num_components; icomp++) {
        HeightInBlocks[icomp] = AUtils::div_roundup<unsigned int>(
            DeCInfo.image_height * DeCInfo.comp_info[icomp].v_samp_factor, 8 * max_v_samp_factor);
        WidthInBlocks[icomp] = AUtils::div_roundup<unsigned int>(
            DeCInfo.image_width * DeCInfo.comp_info[icomp].h_samp_factor, 8 * max_h_samp_factor);
    }
...
unsigned long JpegFile::getNumSamples(void) const { return StegoIndices.size(); }
