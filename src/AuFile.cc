...
void AuFile::read(BinaryIO *io) {
    ...
    try {
        ...
        default:
            if (getBinIO()->is_std()) {
                throw NotImplementedError(
                    _("the au file on standard input uses the unknown encoding %d."), encoding);
            } else {
        ...
        case BinaryInputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while reading the au headers from the file \"%s\"."),
        ...
        case BinaryInputError::STDIN_ERR: {
            throw SteghideError(
                _("an error occurred while reading the au headers from standard input."));
            break;
        }

        case BinaryInputError::STDIN_EOF: {
            throw SteghideError(
                _("premature end of data from standard input while reading au headers."));
            break;
        }
        }
    }
...
void AuFile::write() {
    ...
    try {
        ...
        case BinaryOutputError::FILE_ERR: {
            throw SteghideError(
                _("an error occurred while writing the au headers to the file \"%s\"."),
                getBinIO()->getName().c_str());
            break;
        }
        ...
        case BinaryOutputError::STDOUT_ERR: {
            throw SteghideError(
                _("an error occurred while writing the au headers to standard output."));
            break;
        }
        }
    }
