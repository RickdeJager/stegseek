...
BmpWinFileTest::BmpWinFileTest(TestSuite *s) : CvrStgFileTest("BmpWinFile", s) {
...
void BmpWinFileTest::setup() {
    ...
    bs3 = new BitString(std::string("yet another test which is even longer than "
                                    "the previous test bitstring"));
    gl3 = Globs;
...
void BmpWinFileTest::testReadWrite() {
    Globs = gl1;
    addTestResult(genericTestReadWrite(std::string(DATADIR) + "win3x1_std.bmp"));
    Globs = gl2;
    addTestResult(genericTestReadWrite(std::string(DATADIR) + "win3x4_std.bmp"));
    Globs = gl3;
    addTestResult(genericTestReadWrite(std::string(DATADIR) + "win3x8_std.bmp"));
    Globs = gl4;
    addTestResult(genericTestReadWrite(std::string(DATADIR) + "win3x24_std.bmp"));
}

void BmpWinFileTest::testReadEmbedExtract() {
    Globs = gl1;
    addTestResult(genericTestReadEmbedExtract(std::string(DATADIR) + "win3x1_std.bmp", *bs1));
    Globs = gl2;
    addTestResult(genericTestReadEmbedExtract(std::string(DATADIR) + "win3x4_std.bmp", *bs2));
    Globs = gl3;
    addTestResult(genericTestReadEmbedExtract(std::string(DATADIR) + "win3x8_std.bmp", *bs3));
    Globs = gl4;
    addTestResult(genericTestReadEmbedExtract(std::string(DATADIR) + "win3x24_std.bmp", *bs4));
}

void BmpWinFileTest::testReadEmbedWriteReadExtract() {
    Globs = gl1;
    addTestResult(
        genericTestReadEmbedWriteReadExtract(std::string(DATADIR) + "win3x1_std.bmp", *bs2));
    Globs = gl2;
    addTestResult(
        genericTestReadEmbedWriteReadExtract(std::string(DATADIR) + "win3x4_std.bmp", *bs3));
    Globs = gl3;
    addTestResult(
        genericTestReadEmbedWriteReadExtract(std::string(DATADIR) + "win3x8_std.bmp", *bs4));
    Globs = gl4;
    addTestResult(
        genericTestReadEmbedWriteReadExtract(std::string(DATADIR) + "win3x24_std.bmp", *bs1));
}
...
void BmpWinFileTest::testEmbeddedValue() {
    Globs = gl1;
    addTestResult(genericTestEmbeddedValue(f1));
    Globs = gl2;
    addTestResult(genericTestEmbeddedValue(f2));
    Globs = gl3;
    addTestResult(genericTestEmbeddedValue(f3));
    Globs = gl4;
    addTestResult(genericTestEmbeddedValue(f4));
}
