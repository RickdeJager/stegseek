...
bool CvrStgFileTest::genericTestPosition(const CvrStgFile *f, const SamplePos pos,
    ...
    if (ArgVerbose && !retval) {
        ...
        std::cerr << "file->getSampleValue (" << pos << ") has key: " << std::hex << sv_o->getKey()
                  << std::dec << std::endl;
...
bool CvrStgFileTest::genericTestSVALCalculation(const CvrStgFile *f, const Graph *g) const {
    ...
    std::vector<SampleValueAdjacencyList *> svals_generic =
        f->CvrStgFile::calcSVAdjacencyLists(svs);
